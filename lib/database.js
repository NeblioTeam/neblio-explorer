...
function find_token(tokenId, cb) {
...
function find_all_proposals(cb) {
...
function find_votes_for_active_proposals(cb) {
  Proposal.find({"status": "active"}, function(err, proposals) {
    if(proposals.length > 1) {
      votes = []
      lib.syncLoop(proposals.length, function (loop) {
        var i = loop.iteration();
        Vote.find({"proposal_id": proposals[i]["p_id"]}, function(err, prop_votes) {
          console.log(prop_votes)
          votes.concat(prop_votes)
        });
        loop.next();
      });
      return cb(votes);
...
function find_block_vote(hash, cb) {
  Vote.findOne({block_hash:hash}, function(err, vote) {
    if(vote) {
      return cb(vote);
    } else {
      return cb();
...
function find_block_votes_by_address(address, cb) {
  Vote.find({staker_addr:address}, function(err, votes) {
    if(votes) {
      return cb(votes);
    } else {
      return cb();
...
function update_address(hash, txid, amount, tokens, type, cb) {
    ...
    if (address) {
      ...
      if ( hash == 'coinbase' ) {
        ...
        lib.is_unique(address.txs, txid, function(unique, index) {
          ...
          if (type == 'vin') {
            sent = sent + amount;
            set_tokens_vin_amount(tokens, function(tokens) {});
          } else {
          ...
          if (unique == true) {
            ...
            Address.update({a_id:hash}, {
              ...
              update_address_tokens(hash, address, tokens, type, function() {
                return cb();
              });
            });
          } else {
            if (type == tx_array[index].type) {
              // Even if we have already processed this txn for one token type,
              // we need to update token balances for all types
              update_address_tokens(hash, address, tokens, type, function() {
                return cb();
              });
            } else {
...
function set_tokens_vin_amount(tokens, cb) {
  if (tokens.length > 1) {
    lib.syncLoop(tokens.length, function (loop) {
      var i = loop.iteration();
      tokens[i].amount = (-1) * tokens[i].amount;
      loop.next();
    });
  }
  cb(tokens);
...
function update_address_tokens(hash, address, tokens, type, cb) {
  lib.syncLoop(tokens.length, function (loop) {
    var i = loop.iteration();

    // if this address already has en entry for this token, add our token amount to it
    Address.update({a_id:hash, 'tokens.id':tokens[i].id}, {
      '$inc': {'tokens.$.amount': tokens[i].amount}
    }, function(err, numAffected){
      console.log('Attempted to add '+tokens[i].amount+' to '+tokens[i].id+' at address '+hash);
      if (err) {
        console.log(err);
      } else {
        console.log(numAffected);
      }
      // if this address does NOT have an entry for this token, push a new one
      Address.update({a_id:hash, 'tokens.id':{$ne: tokens[i].id}}, {
        '$push': {
          'tokens': {id:tokens[i].id, amount:tokens[i].amount, meta:tokens[i].meta}
        }
      }, function(err, numAffected){
        console.log('Pushed token '+tokens[i].id+' at address '+hash);
        if (err) {
          console.log(err);
        } else {
          console.log(numAffected);
    ...
    update_token(tokens[i].id, function() {});
    loop.next();
    });
  return cb();
}

function update_token(tokenId, cb) {
  lib.get_ntp1_tokenmetadata(tokenId, function(metadata) {
    find_token(tokenId, function(token) {
      if (token) {
        Token.update({t_id:tokenId}, {
          total_supply: metadata.totalSupply,
          num_holders: metadata.numOfHolders,
          num_transfers: metadata.numOfTransfers,
          num_issuance: metadata.numOfIssuance,
          num_burns: metadata.numOfBurns,
        }, function() {
          return cb();
        });
      } else {
        var meta_of_iss = {};
        if (metadata.metadataOfIssuence) {
          meta_of_iss = metadata.metadataOfIssuence;

          if(!meta_of_iss.data.description) {
              meta_of_iss.data.description = "";
          }
          if(!meta_of_iss.data.issuer) {
              meta_of_iss.data.issuer = "";
          }
          if(!meta_of_iss.data.tokenName) {
              meta_of_iss.data.tokenName = "";
          }

          var newToken = new Token({
            t_id: tokenId,
            aggregation_policy: metadata.aggregationPolicy,
            lock_status: metadata.lockStatus,
            ...
            num_issuance: metadata.numOfIssuance,
            num_burns: metadata.numOfBurns,
            first_block: metadata.firstBlock,
            issuance_txid: metadata.issuanceTxid,
            issuance_address: metadata.issueAddress,
            meta_of_issuance: meta_of_iss,
          });
          ...
          newToken.save(function(err) {
            if (err) {
              ...
              //console.log('token saved: %s', tokenId);
...
function save_tx(txid, cb) {
  ...
  lib.get_rawtransaction(txid, function(tx){
    if (tx && tx.blockhash && tx != 'There was an error. Check your console.') {
      lib.get_block(tx.blockhash, function(block){
        if (block) {
          // Retrieve NTP1 transaction
          lib.get_ntp1_transaction(txid, function(tx_ntp1){
            tx_ntp1.has_token = false;
            lib.prepare_vin(tx, tx_ntp1, function(vin) {
              if (vin.length) {
                lib.syncLoop(vin.length, function (loop) {
                  var i = loop.iteration();
                  if(vin[i].tokens && vin[i].tokens.length > 0) {
                    tx_ntp1.has_token = true;
                    loop.next();
                  } else {
                    loop.next();
                  }
              ...
              lib.prepare_vout(tx.vout, tx_ntp1, txid, vin, function(vout, nvin) {
                lib.syncLoop(vin.length, function (loop) {
                  var i = loop.iteration();
                  var nvin_tokens = [];
                  if (nvin[i].tokens) {
                    nvin_tokens = nvin[i].tokens;
                  }
                  update_address(nvin[i].addresses, txid, nvin[i].amount, nvin_tokens, 'vin', function(){
                    loop.next();
                  });
                }, function(){
                  lib.syncLoop(vout.length, function (subloop) {
                    ...
                    if (vout[t].addresses) {
                      var vout_tokens = [];
                      if (vout[t].tokens) {
                        vout_tokens = vout[t].tokens;
                        tx_ntp1.has_token = true;
                      }

                      update_address(vout[t].addresses, txid, vout[t].amount, vout_tokens, 'vout', function(){
                        subloop.next();
                      });
                    } else {
                      subloop.next();
...
function get_market_data(market, cb) {
  ...
  switch(market) {
    case 'cmc':
      cmc.get_data(settings.markets.coin, settings.markets.exchange, settings.markets.cmc_api_key, function (err, obj) {
        return cb(err, obj);
      });
      break;
    case 'bittrex':
...
module.exports = {
  ...
  get_token: function(tokenId, cb) {
    find_token(tokenId, function(token){
      return cb(token);
    });
  },

  get_tokens: function(cb) {
    Token.find({}).sort({first_block: 'desc'}).exec(function(err, tokens) {
      if (err) {
        return cb(err);
      } else {
        return cb(tokens);
  ...
  get_block_vote: function(hash, cb) {
    find_block_vote(hash, function(vote){
      return cb(vote);
    });
  },

  get_block_votes_by_address: function(address, cb) {
    find_block_votes_by_address(address, function(votes){
      return cb(votes);
    });
  },

  get_all_proposals: function(cb) {
    find_all_proposals(function(proposals){
      return cb(proposals);
    });
  },

  get_votes_for_active_proposals: function(cb) {
    find_votes_for_active_proposals(function(votes){
      return cb(votes);
    });
  },
  ...
  get_distribution: function(richlist, stats, cb){
    ...
    lib.syncLoop(richlist.balance.length, function (loop) {
      ...
      // skipping entry 0, the burn address
      // skipping entry 1, the binance address
      if (count <= 25 && i > 1) {
        ...
        Markets.update({market:market}, {
          ...
          if ( market == settings.markets.default ) {
            Stats.update({coin:settings.coin}, {
              last_price_btc: obj.stats.btc,
              last_price_usd: obj.stats.usd,
            }, function(){
              return cb(null);
            });
          } else {
