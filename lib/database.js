...
function update_address_tokens(hash, addr, tokens, type, cb) {
  if (tokens.length > 0) {
    lib.syncLoop(tokens.length, function (loop) {
      var i = loop.iteration();
      var t_found = false;
      find_address(addr.a_id, function(address) {
        if (address && address.tokens && address.tokens.length) {
          lib.syncLoop(address.tokens.length, function (subloop) {
            var t = subloop.iteration();
            if (tokens[i].id == address.tokens[t].id) {
              t_found = true;
              console.log(address.tokens[t].amount);
              console.log(tokens[i].amount);
              amount = address.tokens[t].amount + tokens[i].amount;
              console.log("Updating token amount to " + amount + " for token " + tokens[i].id + " at address " + hash);
              Address.update({a_id:hash, 'tokens.id':tokens[i].id}, {
                 'tokens.$.amount': amount
              }, function() {});
            }
            // if we are at the end of the subloop, check to see if we found the token
            if (t == address.tokens.length) {
              if (t_found) {
