  ...
  if (tokens.length > 0) {
    lib.syncLoop(tokens.length, function (loop) {
      ...
      if (!t_found) {
        Address.update({a_id:hash}, {
          '$push': {
            'tokens': {id:tokens[i].id, amount:tokens[i].amount}
          }
        }, function() {});
      }
      if (type == 'vin') {
  ...
  lib.get_ntp1_tokenmetadata(tokenId, function(metadata) {
    find_token(tokenId, function(token) {
      if (token) {
        ...
        var issuance = {};
        if (metadata.metadataOfIssuance) {
          issuance = metadata.metadataOfIssuance;
        }
        ...
        if (block) {
          ...
          lib.get_ntp1_transaction(txid, function(tx_ntp1){
            lib.prepare_vin(tx, tx_ntp1, function(vin) {
              if (vin.length > 0 && vin[0].tokens && vin[0].tokens.length > 0) {
...
module.exports = {
  ...
  get_token: function(tokenId, cb) {
    find_token(t_id, function(token){
      return cb(token);
    });
  },
